...
def _run_until_complete_cb(fut):  # 用来结束循环
    exc = fut._exception
...
class BaseEventLoop(events.AbstractEventLoop):
    ...
    def create_task(self, coro):  # Task继承自Future，就把task看成是一个future吧
        """Schedule a coroutine object.

        Return a task object.
        """
        self._check_closed()
        if self._task_factory is None:  # 应该是说指定创造task的类，如果没指定，则用Task创建，指定了就用指定的
    ...
    def call_later(self, delay, callback, *args):
        ...
        """
        """
        call_later就相当于注册了一个回调，类似于yield_from.py模型中
        的selector.register(sock.fileno(), EVENT_WRITE, on_connected)，当时间一到
        就好比是上述中网络连接成功事件发生，随即把该事件加入轮询中，随后调用on_connected，
        这里就是时间一到，就会在_run_once()中把事件加入轮询（即_scheduled那一块的代码）
        """
    ...
    def call_at(self, when, callback, *args):
        ...
        timer = events.TimerHandle(when, callback, args, self)  # timer其实就是yield_from.py中的sock.fileno()，其中注册了callback
        if timer._source_traceback:
            del timer._source_traceback[-1]
    ...
    def call_soon(self, callback, *args):
        ...
        看future.Future中的_schedule_callbacks和add_done_callback就能明白call_soon的含义，
        即，把待调用的回调函数（已就绪事件的回调函数）安排到_ready队列中，然后在_run_once中调
        用各handle._run方法执行回调函数
        """
        self._check_closed()
        if self._debug:
            self._check_thread()
    ...
    def _run_once(self):
        ...
        sched_count = len(self._scheduled)  # 计算堆中有多少个定时任务
        ...
        if self._ready or self._stopping:
            timeout = 0
        elif self._scheduled:  # sleep时间到了就会把待调用事件添加到_scheduled
        ...
        while self._scheduled:  # sleep时间到了的都从heappop弹出，添加到self._ready中，在本帧轮询中调用
